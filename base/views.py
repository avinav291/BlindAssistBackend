...
def uploadImage(request):
    ...
    headers = {'Content-Type': 'application/json', 'app_id': '9e6b035a',
               'app_key': 'be6b4c0eab5d802b243bd6c9d3a6ad0a'}  # TODO Change Keys
    myfile = request.FILES.get('image')
    fs = FileSystemStorage()
    filename = fs.save("media/" + myfile.name, myfile)
    with open(filename, "rb") as image_file:
        encoded_string = base64.b64encode(image_file.read())
        payload = {"image": encoded_string, "gallery_name": "Gallery", "subject_id": name} #TODO Change gallery name
...
def uploadImages(request):
    ...
    for i in range(1, 7):
        ...
        if request.FILES.has_key('image'+str(i)):
            ...
            with open(filename, "rb") as image_file:
                encoded_string = base64.b64encode(image_file.read())
                payload = {"image": encoded_string, "gallery_name": "Gallery", "subject_id": name}
...
def sendImage(image):
    ...
    with open(image, "rb") as image_file:
        ...
        payload = {"image": encoded_string, "gallery_name": "Gallery"} #TODO Change gallery name before demo
        headers = {'Content-Type': 'application/json', 'app_id': '9e6b035a',
                   'app_key': 'be6b4c0eab5d802b243bd6c9d3a6ad0a'}   #TODO Change Keys
        # POST with JSON
